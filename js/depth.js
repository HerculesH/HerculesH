...
function predict() {
  ...
  setTimeout(function() {
    ...
    const getPortraitDepth = async () => {
      ...
      predictButton.textContent = 'Latency';
...
async function setupPage() {
  ...
  const dropzoneForm = document.getElementById('dropzoneButton');
  dropzoneForm.addEventListener('change', (event) => {
    const fileList = event.target.files;
    changeImage(fileList[0], 'im1');
  // handle submit
});
